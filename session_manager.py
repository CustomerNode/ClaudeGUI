...
def api_new_session():
    """Create a blank session file with a known ID, launch claude --resume on it."""
    import uuid as _uuid
    try:
        proj_dir = _decode_project(_active_project) if _active_project else str(Path.home())
        new_id = str(_uuid.uuid4())
        # Create a minimal blank session file so claude --resume can find it
        session_path = _sessions_dir() / f"{new_id}.jsonl"
        session_path.write_text("", encoding="utf-8")
        subprocess.Popen(
            f'start /MIN cmd /k "cd /d {proj_dir} && claude --resume {new_id}"',
            shell=True
        )
...
<div class="layout">
  ...
  <div class="sidebar">
    <div class="sidebar-toolbar">
      <button id="btn-new-session" onclick="newSession()" style="width:100%;padding:8px;font-size:13px;font-weight:600;background:#1a3a2a;border:1px solid #2a6a4a;color:#66ddaa;border-radius:7px;cursor:pointer;letter-spacing:.03em;" title="Start a new Claude session">+ New Session</button>
...
async function newSession() {
  ...
  const name = await showNamePrompt('Name this session', 'e.g. Customer Analysis, Bug Fix…');
  ...
  try {
    ...
    if (d.ok && d.new_id) {
      if (name) {
        await fetch('/api/rename/' + d.new_id, {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
      ...
      await loadSessions();
      runningIds.add(d.new_id);
      sessionKinds[d.new_id] = 'idle';
      filterSessions();
      await openInGUI(d.new_id);
    } else {
      showToast('Failed: ' + (d.error || 'unknown'), true);
    }
  } finally {
...
function showNamePrompt(title, placeholder) {
  return new Promise(resolve => {
    // Remove any existing prompt
    const existing = document.getElementById('name-prompt-overlay');
    ...
    overlay.innerHTML = `
      <div style="background:#1a1a2e;border:1px solid #44447a;border-radius:12px;padding:24px 28px;min-width:340px;max-width:480px;box-shadow:0 8px 40px rgba(0,0,0,.8);">
        <div style="font-size:14px;font-weight:600;color:#c8c8ff;margin-bottom:14px;">${title}</div>
        <input id="name-prompt-input" type="text" placeholder="${placeholder}"
          style="width:100%;box-sizing:border-box;background:#0e0e1e;border:1px solid #44447a;border-radius:7px;color:#eee;font-size:13px;padding:9px 12px;outline:none;">
        <div style="display:flex;gap:8px;margin-top:14px;justify-content:flex-end;">
          <button id="name-prompt-skip" style="padding:7px 16px;background:#1e1e2e;border:1px solid #333;color:#888;border-radius:6px;cursor:pointer;font-size:12px;">Skip</button>
          <button id="name-prompt-ok" style="padding:7px 18px;background:#1a3a2a;border:1px solid #2a6a4a;color:#66ddaa;border-radius:6px;cursor:pointer;font-size:12px;font-weight:600;">Start Session</button>
        </div>
      </div>`;
    ...
    document.getElementById('name-prompt-ok').onclick = () => finish(input.value.trim());
    document.getElementById('name-prompt-skip').onclick = () => finish('');
...
function getSessionStatus(id) {
  if (!runningIds.has(id)) {
    if (waitingData[id]) return 'question';  // has a pending question — definitely active
    ...
    try:
        ...
        for proc in data:
            ...
            if name not in ("node.exe", "claude.exe", "claude"):
                continue
            # Skip VS Code extension claude (has --output-format stream-json)
            ...
            if m:
                ...
                # bare `claude` or `claude --resume` without UUID visible — match by recent file
                unmatched_pids.append(proc.get("ProcessId"))

        # Match unmatched processes to the most recently active .jsonl files not already claimed.
        if unmatched_pids:
            candidates = sorted(
